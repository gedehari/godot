...
float RBAudioPreview::get_length() const {
	return length;
}

float RBAudioPreview::get_max(float p_time, float p_time_next) const {
	if (length == 0) {
		return 0;
	}
...
float RBAudioPreview::get_min(float p_time, float p_time_next) const {
	if (length == 0) {
		return 0;
	}
...
RBAudioPreview::RBAudioPreview() {
	length = 0;
}

////

void RBAudioPreviewGenerator::_update_emit(ObjectID p_id) {
	emit_signal(SNAME("preview_updated"), p_id);
}

void RBAudioPreviewGenerator::_preview_thread(void *p_preview) {
...
Ref<RBAudioPreview> RBAudioPreviewGenerator::generate_preview(const Ref<AudioStream> &p_stream) {
	ERR_FAIL_COND_V(p_stream.is_null(), Ref<RBAudioPreview>());
	...
	if (preview->playback.is_valid()) {
		...
		preview->thread->set_name("RBAudioPreviewGenerator");
...
void RBAudioPreviewGenerator::_bind_methods() {
	ClassDB::bind_method("_update_emit", &RBAudioPreviewGenerator::_update_emit);
	ClassDB::bind_method(D_METHOD("generate_preview", "stream"), &RBAudioPreviewGenerator::generate_preview);

	ADD_SIGNAL(MethodInfo("preview_updated", PropertyInfo(Variant::INT, "obj_id")));
}

RBAudioPreviewGenerator *RBAudioPreviewGenerator::singleton = nullptr;

void RBAudioPreviewGenerator::_notification(int p_what) {
	switch (p_what) {
...
RBAudioPreviewGenerator::RBAudioPreviewGenerator() {
	singleton = this;
	set_process(true);
}
